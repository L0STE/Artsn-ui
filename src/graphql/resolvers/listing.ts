...
export const listingResolvers: IResolvers<any, Context> = {
  Query: {
    ...
    getAllListings: async (_parent: any, { sort, limit, offset }: ListingQueryOptions = {}) => {
      const { db } = await connectToDatabase();
    ...
    getListingsByNetReturn: async (_parent: any, { limit, offset }: { limit?: number; offset?: number } = {}) => {
      const { db } = await connectToDatabase();
    ...
    getRecentListings: async (_parent: any, { limit, offset }: { limit?: number; offset?: number } = {}) => {
      const { db } = await connectToDatabase();
      let query = db.collection('listings')
        .find()
    ...
    getMostSoldListings: async (_parent: any, { limit, offset }: { limit?: number; offset?: number } = {}) => {
      const { db } = await connectToDatabase();
      let query = db.collection('listings')
        .find()
        .sort({ sold: -1 } as Sort);
