...
export const useStore = create<StoreState>()(
  persist(
    (set, get) => ({
      ...
      setUserLoading: (userId, loading) => set((state) => ({
        loadingStates: {
          ...state.loadingStates,
      ...
      // Listing actions
      upsertListing: (listing) => set((state) => ({
        listings: new Map(state.listings).set(listing._id, listing)
      })),
      
      upsertListings: (listings) => set((state) => ({
        listings: new Map<string, ListingType>([
          ...Array.from(state.listings.entries()),
          ...listings.map(listing => [listing._id, listing] as [string, ListingType])
        ])
      })),
      ...
      setListingLoading: (listingId, loading) => set((state) => ({
        loadingStates: {
          ...state.loadingStates,
          listings: new Map(state.loadingStates.listings).set(listingId, loading)
        }
      })),
