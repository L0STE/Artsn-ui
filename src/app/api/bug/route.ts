...
const handler = async(req: NextRequest) => {
    ...
    try {
        const scriptUrl = 'https://script.google.com/macros/s/AKfycbypkHmIgZ0aTEVIXQbgLRNydRVgt2CEFZKIPahJGaQykEhFvo_CmqG-oypyp_R_1JT8/exec';
        ...
        if (!response.ok) {
            if (response.status === 401) {
                ...
                return new Response('Authentication failed with Google Script. Please check your deployment settings.', {
                    status: 401,
                    headers: { 'Content-Type': 'text/plain' },
                });
            }
            ...
            return new Response(`Failed to submit bug report: ${response.status} ${response.statusText}`, {
        ...
        try {
            ...
            return new Response('Invalid response from Google Script. Please check the script configuration.', {
                status: 500,
                headers: { 'Content-Type': 'text/plain' },
            });
        }
